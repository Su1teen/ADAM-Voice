  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="fixed inset-0 bg-gradient-to-br from-black via-[#0a0a0a] to-black pointer-events-none">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-blue-900/10 via-transparent to-transparent opacity-50" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-purple-900/10 via-transparent to-transparent opacity-30" />
      </div>
      
      {/* Top-right navigation buttons with Liquid Glass */}
      <motion.nav
        className="fixed top-4 right-4 z-40 flex flex-wrap gap-2 md:gap-3 max-w-[calc(100vw-2rem)]"
      ...
      >
        <motion.button
          ...
          className={`glass-button-compact ${showChat ? 'bg-blue-500/20 border-blue-500/50' : ''}`}
        ...
        <motion.button
          ...
          className={`glass-button-compact ${showSmartHome ? 'bg-green-500/20 border-green-500/50' : ''}`}
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          title="Умный дом"
        >
        ...
        <motion.button
          ...
          className={`glass-button-compact ${showHealthInsights ? 'bg-purple-500/20 border-purple-500/50' : ''}`}
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
        ...
        <motion.button
          ...
          className={`glass-button-compact ${showBuildingAutomation ? 'bg-orange-500/20 border-orange-500/50' : ''}`}
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          title="Автоматизация"
        >
          <Layers size={18} />
          <span className="hidden sm:inline">Автом.</span>
        </motion.button>

        <motion.button
          onClick={() => setShowCustomization(!showCustomization)}
          className={`glass-button-compact ${showCustomization ? 'bg-gray-500/20 border-gray-500/50' : ''}`}
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          title="Настройки"
        >
      ...
      {/* Voice visualizer - centered and responsive */}
      <motion.div
        className="fixed inset-0 flex items-center justify-center pointer-events-none z-10 px-4"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <AnimatePresence mode="wait">
        {/* Chat overlay - mobile responsive */}
        {showChat && (
          <motion.div
            key="chat"
            ...
            exit={{ opacity: 0 }}
          >
            <div className="absolute inset-0 bg-black/85 backdrop-blur-lg" onClick={() => setShowChat(false)} />
            <motion.div
              className="relative w-full max-w-4xl h-[92vh] sm:h-[85vh]"
        ...
        {/* Smart Home Panel - mobile responsive */}
        {showSmartHome && (
          <motion.div
            key="smarthome"
            className="fixed inset-0 z-30 flex items-center justify-center p-2 sm:p-4"
            ...
            exit={{ opacity: 0 }}
          >
            <div className="absolute inset-0 bg-black/85 backdrop-blur-lg" onClick={() => setShowSmartHome(false)} />
            <motion.div
              className="relative w-full max-w-6xl h-[92vh] sm:h-[85vh]"
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              transition={{ type: 'spring', damping: 30, stiffness: 300 }}
        ...
        {/* Productivity & Health Panel - mobile responsive */}
        {showHealthInsights && (
          <motion.div
            key="productivity"
            className="fixed inset-0 z-30 flex items-center justify-center p-2 sm:p-4"
            ...
            exit={{ opacity: 0 }}
          >
            <div className="absolute inset-0 bg-black/85 backdrop-blur-lg" onClick={() => setShowHealthInsights(false)} />
            <motion.div
              className="relative w-full max-w-6xl h-[92vh] sm:h-[85vh]"
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
        ...
        {/* Building Automation Panel - mobile responsive */}
        {showBuildingAutomation && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <div className="absolute inset-0 bg-black/85 backdrop-blur-lg" onClick={() => setShowBuildingAutomation(false)} />
            <motion.div
              className="relative w-full max-w-6xl h-[92vh] sm:h-[85vh]"
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
        ...
        {/* Customization Panel - mobile responsive */}
        {showCustomization && (
          <motion.div
            key="settings"
            className="fixed inset-0 z-30 flex items-center justify-center p-2 sm:p-4"
            ...
            exit={{ opacity: 0 }}
          >
            <div className="absolute inset-0 bg-black/85 backdrop-blur-lg" onClick={() => setShowCustomization(false)} />
            <motion.div
              className="relative w-full max-w-6xl h-[92vh] sm:h-[85vh]"
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
      ...
      {/* Chat input - mobile responsive at bottom */}
      <div className="fixed bottom-0 left-0 right-0 z-20 px-2 sm:px-4 pb-2 sm:pb-4">
        <ChatInput
